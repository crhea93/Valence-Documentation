...
def index(request):
    """
    Function that loads the CAM drawing page. The function checks that the user is authenticated
    (if not the user is redirected to the login page)
    and the obtains their active CAM. It then pulls the blocks and links associated with the CAM.
    Finally, it passes that information to templates/base/index.html for rendering.
    """
...
def loginpage(request):
    """
    View that renders the basic login page. The view accepts information from the basic AuthenticationForm
    defined in django.auth. The username and password are passed to this view from templates/registration/login.html.
    If the user is authenticated, they are redirected to the dashboard. The view also checks if the password is correct,
    that the user exists, and that the user has been authenticated. If any of these errors occur, it will return an error
    message that will display on the login page.
    """
    if request.method == 'POST':
...
def create_participant(request):
    ...
    This is called from template/registration/register_participant.html.
    """
    if request.method == 'POST':
...
def create_researcher(request):
    ...
    the user specifically signs up as a researcher. This is called from template/registration/register_researcher.html.
    """
    if request.method == 'POST':
...
def clear_CAM(request):
    ...
    Function to clear a CAM. This function simply deletes all the blocks and links in a current CAM. After this function,
     ...
     This is called from templates/Admin/reset.html.
    """
...
def Image_CAM(request):
    """
    Function that creates an image of the CAM directly from the drawing tool.
    For the moment, this function obtains an HTML canvas created with the canvastoHTML tool.
    The canvas is then translated into a pdf image. This image is written to
    'media/CAMS/'+request.user.username+'_'+str(user.active_cam_num)+'.pdf'.

    This function is called from templates/base/image-save.html or templates/base/image-save.html
    """
    ...
    #str_html = '<!DOCTYPE html> <html> <body>' + image_data + '</body> </html>'
    user = CustomUser.objects.get(username=request.user.username)
    file_name = 'media/CAMS/'+request.user.username+'_'+str(user.active_cam_num)+'.pdf'
    #pdfkit.from_string(str_html, file_name)
    #print(str_html)
    pdf = makepdf(image_data)
    Path(file_name).write_bytes(pdf)
    current_cam = CAM.objects.get(id=user.active_cam_num)
    current_cam.cam_image = file_name
    current_cam.save()
    #print(image_data)
